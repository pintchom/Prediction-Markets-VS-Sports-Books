#!/usr/bin/env python3
"""
Script to collect Kalshi market prices 5 minutes before game start (closing lines)
...
def get_price_before_game_start(series_ticker, market_ticker, expected_expiration_time, minutes_before=5):
    ...
    Args:
        ...
        market_ticker: Market ticker (e.g., 'KXNFLGAME-25OCT26BUFCAR-BUF')
        expected_expiration_time: ISO timestamp from Kalshi (e.g., '2025-10-26T20:00:00Z')
    ...
    """
    try:
        # Calculate game start time (expiration - 3 hours)
        game_end = datetime.fromisoformat(expected_expiration_time.replace('Z', '+00:00'))
        game_start = game_end - timedelta(hours=3)
        ...
        url = f"https://api.elections.kalshi.com/trade-api/v2/series/KXNFLGAME/markets/{market_ticker}/candlesticks"
...
def collect_closing_lines_for_all_games(markets_file, output_file, delay=0.1):
    """
    Collect closing lines (5 min before start) for all NFL games
...
def main():
    """Main execution"""
    markets_file = DATA_DIR / "nfl_series_markets.json"
    output_file = DATA_DIR / "nfl_closing_lines.json"
