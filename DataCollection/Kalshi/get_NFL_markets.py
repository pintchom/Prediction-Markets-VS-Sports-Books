...
def fetch_nfl_series_markets():
    """
    Fetch all markets for each NFL series from game_series.json
    """
    print("\nLoading NFL series from game_series.json...")
    
    # Read the sports series file
    with open(GAME_SERIES_FILE, 'r') as f:
        sports_data = json.load(f)
    
    nfl_series = sports_data.get('nfl_series', [])
    print(f"Found {len(nfl_series)} NFL series to process")
    ...
    # Process each series
    for i, series in enumerate(nfl_series, 1):
        ticker = series['ticker']
        title = series['title']
        
        print(f"\n[{i}/{len(nfl_series)}] Processing {title} ({ticker})...")
...
def main():
    ...
    """
    print("=" * 60)
    print("NFL Series Markets Collection Script")
    print("=" * 60)
    
    try:
        # Fetch all markets for NFL series (no authentication needed for public endpoints)
        all_series_markets, total_markets = fetch_nfl_series_markets()
        ...
        # Save to JSON file
        output_file = os.path.join(OUTPUT_DIR, "nfl_series_markets.json")
        ...
        print("=" * 60)
        print(f"\nTotal NFL Series: {len(all_series_markets)}")
