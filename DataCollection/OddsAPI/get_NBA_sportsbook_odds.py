#!/usr/bin/env python3
"""
Script to collect sportsbook odds 5 minutes before NBA game start
...
def collect_sportsbook_odds_for_games(kalshi_data_file: str, output_file: str, delay: float = 0.5):
    """
    Collect sportsbook odds for NBA games based on Kalshi market data
    ...
    # NBA team mapping from abbreviations to full names
    # Note: The Odds API uses "Los Angeles Clippers" not "LA Clippers"
    team_map = {
    ...
    for kalshi_game in closing_lines:
        ...
        if subtitle and (' @ ' in subtitle or ' vs ' in subtitle):
            ...
            # Parse from ticker: KXNBAGAME-25NOV23CHAATL-CHA
            # Format is usually KXNBAGAME-{date}{team1}{team2}-{winner}
            try:
                parts = ticker.split('-')
                if len(parts) >= 3:
                    team_part = parts[1][7:]  # Remove date part (25NOV23)
                    winner = parts[2]
    ...
    output_data = {
        ...
        'description': 'Sportsbook odds 5 minutes before NBA game start',
        'source': 'The Odds API',
...
def main():
    """Main execution"""
    kalshi_data_file = DATA_DIR / "nba_closing_lines.json"
    output_file = DATA_DIR / "nba_sportsbook_odds.json"
